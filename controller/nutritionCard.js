...
exports.displayNutritionCards = function getNutritionData(foodName, session){
    var url = "https://api.nal.usda.gov/ndb/search/?format=json&q="+foodName+"&sort=r&max=1&offset=0&api_key=oSGlFybZfFnQ1WRjCOl5hSQ0JGwH8eQg5XPhbuqt";//change the api key 
...
function getFoodNutrition(message, foodName, session){
    ...
    var ndbno = foodNutritionList.list.item[0].ndbno;//callback function for the food ndbno is the food nutration value 
    var url = "https://api.nal.usda.gov/ndb/reports/?ndbno="+ndbno+"&type=f&format=json&api_key=oSGlFybZfFnQ1WRjCOl5hSQ0JGwH8eQg5XPhbuqt";//change the api key 
...
function displayNutritionCards(message, foodName,session){//this is the callback function 
    //Parses JSON
